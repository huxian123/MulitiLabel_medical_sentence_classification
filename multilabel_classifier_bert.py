...
# In[5]:
#加载bert字库文件、预训练模型、配置文件
BERT_VOCAB= 'models/vocab.txt'
BERT_INIT_CHKPNT = 'models/bert_model.ckpt'
BERT_CONFIG = 'models/bert_config.json'
...
train = train[['ID','Question Sentence','category_A','category_B','category_C','category_D','category_E','category_F']]
print(train.head(5))
...
def create_model_focal(bert_config, is_training, input_ids, input_mask, segment_ids,
                 labels, num_labels, use_one_hot_embeddings):
    model = modeling.BertModel(
        config=bert_config,
        is_training=is_training,
...
test_file = os.path.join('./output', "test.tf_record")
#filename = Path(train_file)
...
#将probality概率数据0-1之间的数字转为0或者1
import numpy as np
output_df = np.array(output_df)
result = np.uint8(output_df > 0.5)
print(result)
...
#形成提交结果
merged_df =  pd.concat([x_test, 
                        pd.DataFrame(result, columns=['category_A','category_B','category_C','category_D','category_E','category_F'])], axis=1)
submission = merged_df.drop(['Question Sentence'], axis=1)
submission.to_csv("data/sample_submission1.csv", index=False)